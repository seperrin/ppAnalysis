...
void PreciseMassStudies(){
    ...
    string SignalF = "CB2-Run2";
    string BackgroundF = "DoubleExpo";
    double min = 2.4;
    double max = 4.7;
    double ratsigma = 1.05;
    ...
    sprintf(FitFileName,"~/../../Volumes/Transcend2/ppAnalysis/Scripts/FitFiles/Septembre2021-Run2ReferencesTKLSystematicsStart/FitFile_NewAnalysisAllEst_Run2_SPDTrackletsPercentile_0-5_40-100_pt0-2-4-6-8-12_ManuInvMass.root");
...
//                if(MinInvMass + 3*(0.5+bin)/NbinsDimuInvMass > 2.5 && MinInvMass + 3*(0.5+bin)/NbinsDimuInvMass < 3.5){
 ...
 //  histo->Fit("fitFcn","0");
   // First fit, fix MJPsi and Sigma
     ...
     if(SignalF=="CB2-Run2" || SignalF=="CB2-MC" || SignalF=="CB2-FREE"){
         ...
         fitFcn->FixParameter(1,3.0975); // Mean x core //mJpsi
         fitFcn->FixParameter(2,0.0695);
    ...
    if(BackgroundF =="VWG"){
        ...
        fitFcn->SetParameter(nextPar+0,30000);
        fitFcn->SetParameter(nextPar+1,1.4);
        fitFcn->SetParameter(nextPar+2,0.4);
        fitFcn->SetParameter(nextPar+3,0.3);
    }
   ...
   // AE  fitFcn->ReleaseParameter(1);
   res = histo->Fit("fitFcn","SBMER","ep");
    // AE fitFcn->ReleaseParameter(2);
   res = histo->Fit("fitFcn","SBMER","ep");
//    if(SignalF=="CB2-Run2" || SignalF=="CB2-MC" || SignalF=="CB2-FREE"){
...
//             fitFcn->SetParameter(0,3300);
//
//             fitFcn->FixParameter(1,3.0966); // Mean x core
...
//    res = histo->Fit("fitFcn","SBMER","ep");
   ...
   integral = (signalFcnJPsi->Integral(0,5))*NbinsDimuInvMass/(MaxInvMass-MinInvMass);
    ...
    auto covtot = res->GetCovarianceMatrix();
        auto correltot = res->GetCorrelationMatrix();
    ...
    covsgn.Print();
        std::cout << "Matrice corrélation" <<endl;
           correltot.Print();
