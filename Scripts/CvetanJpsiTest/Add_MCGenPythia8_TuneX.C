...
AliGenerator* CreatePythia8Gen(	Float_t e_cms, 
	...
	if(kProcess==2) gener->SetProcess(kPyJpsiChi);

	if(kProcess==3) {
	  gener->SetProcess(kPyBeauty);

	  // (AliPythia8::Instance())->ReadString("HardQCD:hardbbbar = on");
	  // (AliPythia8::Instance())->ReadString("HardQCD:gg2ccbar = off");
	  // (AliPythia8::Instance())->ReadString("HardQCD:qqbar2ccbar = off");
	  // (AliPythia8::Instance())->ReadString("HardQCD:all = off");

	  //	  (AliPythia8::Instance())->ReadString("HardQCD:hardccbar = off");
	  //	  (AliPythia8::Instance())->ReadString("Onia:all = on");
	  //	  (AliPythia8::Instance())->ReadString("Charmonium:all = on");
	  //	  (AliPythia8::Instance())->ReadString("Bottomonium:all = on");
	  //	  (AliPythia8::Instance())->ReadString("HardQCD:all = on");
	  //	  (AliPythia8::Instance())->ReadString("HardQCD:gg2ccbar = off,HardQCD:qqbar2ccbar = off,HardQCD:hardbbbar = on");
	  //	  (AliPythia8::Instance())->ReadString("HardQCD:gg2ccbar = off");
	  //	  (AliPythia8::Instance())->ReadString("HardQCD:qqbar2ccbar = off");
	}
